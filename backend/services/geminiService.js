...
// Helper utility to pause execution for exponential backoff
const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

// ── Raw call ─────────────────────────────────────────────────────────────────
async function callGeminiRaw(prompt, stageName, retryAttempt = 0) {
  ...
  try {
    ...
    // Catch 429 Quota Exceeded and retry locally with a backoff delay
    const isRateLimit = error.status === 429 || 
                        error.message?.includes('429') || 
                        error.message?.includes('Quota exceeded');
                        
    if (isRateLimit && retryAttempt < 3) {
      const waitTime = Math.pow(2, retryAttempt) * 3000; // 3s, then 6s, then 12s
      logger.log(`[GeminiService] ⚠️ Rate limited on ${stageName}. Retrying in ${waitTime / 1000}s...`, 'warn');
      await delay(waitTime);
      return callGeminiRaw(prompt, stageName, retryAttempt + 1);
    }
    throw error;
  }
}
...
async function callAITextRaw(prompt, systemInstruction, retryAttempt = 0) {
  try {
    const response = await ai.models.generateContent({
      model: CHAT_MODEL,
      config: {
    ...
    if (isRateLimit && retryAttempt < 3) {
      const waitTime = Math.pow(2, retryAttempt) * 3000;
      logger.log(`[GeminiService] ⚠️ Text call rate limited. Retrying in ${waitTime / 1000}s...`, 'warn');
      await delay(waitTime);
      return callAITextRaw(prompt, systemInstruction, retryAttempt + 1);
    }
    throw error;
  }
}
